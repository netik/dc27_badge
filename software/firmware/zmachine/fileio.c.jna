...
{
   //printf("in read_page(): "); printf(page); printf(", "); printf((uint32_t) buffer, HEX);
   unsigned long file_size;
   unsigned int pages, offset;

   /* Seek to start of page */
   int seekcode = gfp.seek(story_offset + ( long ) page * PAGE_SIZE);
   //printf("Debug read_page() Offset: ");
   //printf(story_offset + ( long ) page * PAGE_SIZE,HEX);
   //printf("Debug seek returned "); printf(seekcode);
...
#endif
   {
      ...
      if ( ( unsigned int ) page == pages )
      {
         ...
         //printf("Debug read_page() 4");
         int seekcode = gfp.seek(story_offset + ( long ) page * PAGE_SIZE);
         //printf("Debug seek() returned "); printf(seekcode);

         //printf("Debug read_page() 5");
#ifdef USE_ZLIB
   ...
   //printf("done read_page()");
}                               /* read_page */

void read_page_old( int page, void *buffer )
{
   //printf("read_page(");printf(page);printf(")");
   unsigned long file_size;
   unsigned int pages, offset;
